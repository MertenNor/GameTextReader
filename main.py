...
def main():
    ...
    # Checked before GameTextReader() runs (raw file, not app state) so it also
    # catches users upgrading from an older version whose settings.json predates
    # the "ocr_backend" key, not just brand new installs with no settings file at all
    def _settings_missing_ocr_backend():
        if not os.path.exists(APP_SETTINGS_PATH):
            return True
        try:
            with open(APP_SETTINGS_PATH, 'r', encoding='utf-8') as f:
                return 'ocr_backend' not in json.load(f)
        except Exception:
            return True

    needs_ocr_engine_choice = _settings_missing_ocr_backend()
    ...
    def check_tesseract_on_startup():
        # Users who picked RapidOCR don't need Tesseract, so don't nag them about it
        if app.get_selected_ocr_backend() == "rapidocr":
            return
    ...
    # First-run only: let the user pick their OCR engine before the Tesseract
    # check runs, so a RapidOCR choice can actually suppress that check below
    def show_first_run_ocr_choice():
        dialog = tk.Toplevel(app.root)
        dialog.withdraw()
        dialog.title("Choose OCR Engine")
        ...
        tk.Label(frame, text="Choose your OCR engine", font=("Helvetica", 12, "bold")).pack(anchor="w", pady=(0, 10))

        tk.Label(frame, text="Tesseract", font=("Helvetica", 10, "bold")).pack(anchor="w")
        tk.Label(
            frame,
            text=(
                f"{APP_NAME}'s default and most tested text-recognition engine.\n"
                "(Requires installing Tesseract itself)"
            ),
            justify="left", wraplength=380, font=("Helvetica", 9)
        ).pack(anchor="w", pady=(0, 10))

        tk.Label(frame, text="RapidOCR", font=("Helvetica", 10, "bold")).pack(anchor="w")
        tk.Label(
            frame,
            text=(
                "A newer, experimental alternative, not every feature has been tested with it yet.\n"
                "(Built in, no further installation needed)"
            ),
            justify="left", wraplength=380, font=("Helvetica", 9)
        ).pack(anchor="w", pady=(0, 10))
        ...
        tk.Label(
            frame,
            text="You can change this later in Additional Options (top left).",
            justify="left", wraplength=380, font=("Helvetica", 8, "italic"), fg="#555555"
        ...
        def choose(engine):
            app.ocr_backend_var.set(engine)
            app.refresh_ocr_backend_status_label()
            dialog.destroy()
            app.root.after(100, check_tesseract_on_startup)

        tk.Button(
            button_row, text="Use Tesseract\n(Recommended)", width=24, justify="center",
            command=lambda: choose("tesseract")
        ).pack(side="left", padx=(0, 8))
        tk.Button(
            button_row, text="Use RapidOCR\n(Experimental)", width=24, justify="center",
            command=lambda: choose("rapidocr")
        ).pack(side="left")

        dialog.protocol("WM_DELETE_WINDOW", lambda: choose("tesseract"))

        dialog.update_idletasks()
        x = app.root.winfo_x() + (app.root.winfo_width() // 2) - (dialog.winfo_reqwidth() // 2)
        y = app.root.winfo_y() + (app.root.winfo_height() // 2) - (dialog.winfo_reqheight() // 2)
        dialog.geometry(f"+{x}+{y}")
        dialog.deiconify()
        dialog.grab_set()
        dialog.focus_force()

    # Schedule after a short delay to ensure the main window is fully displayed.
    # When the choice hasn't been made yet, the dialog runs first and only then
    # triggers the Tesseract check itself, so ordering is never a race between the two.
    if needs_ocr_engine_choice:
        app.root.after(500, show_first_run_ocr_choice)
    else:
